...
function strfunc(str){ str.replace(/;$/,'').split(';').forEach(function(s){ try{ return Function('"use strict";return ('+s+')')(); }catch(err){console.log('strfunc error running: '+s+'\r>>>'+err)} }) }
...
function initClickEvents(){
	$('#container,.fixed-item-wrap').find('*').each(function(index,el){
        ...
        $.each(el.attributes,function(ind,attrib){
            ...
            switch(at){
                case 'onclick': postArr.push(function(){$el.attr('data-onclick',aval).removeAttr(at);}); clickArr.push(function(event){strfunc($el.attr('data-onclick'));/*name must be 'event'*/ }); break;
            ...
            if($el.parent().hasClass('activePage')) strfunc(aval); }); break;
...
function openLink($a,e){
	...
	if(href.indexOf('javascript:')===0){strfunc(href); w=!0;}
...
(function(){
	...
	_gtag('set', {'appName':"2022 Hue Equity Research Report ",'appId':"state-of-inequity-report",'appVersion':''});
